...
# gpu显存设置
gpu_options = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=0.3)   #设置每个进程使用GPU内存所占的比例

PATH_TO_CKPT = os.path.abspath(os.path.join(os.path.dirname(__file__), "frozen_inference_graph.pb"))
...
class CarDetectot_SSD(object):
    ...
    def __init__(self):
        ...
        with self.detection_graph.as_default():
            with tf.compat.v2.io.gfile.GFile(PATH_TO_CKPT, "rb") as f:
                inference_graph_def = tf.compat.v1.GraphDef()
                inference_graph_def.ParseFromString(f.read())
                tf.import_graph_def(inference_graph_def, name='')

        self.sess = tf.compat.v1.Session(graph=self.detection_graph, config=tf.compat.v1.ConfigProto(gpu_options=gpu_options))
