...
from ssd_detector import CarDetectot_SSD
from Mytools import draw_box_label, box_iou
...
def process_video(handle=None, input_video=None, args=None, ** kwargs):
    ...
    #         img = img[None]
        ...
        x, y, width, height, vechile_type, id = pipeline(bbox, clses)
        ...
        with open(output_tracker_file, 'a+') as tracker_file:
            for i in range(len(id)):
                text = str(frame_id) + "," + str(id[i]) + "," + str(x[i]) + "," + str(y[i]) + "," + str(width[i]) + "," + str(height[i]) + ",1," + str(int(vechile_type[i]) + 1) + ",1\n"
                #print("text:",text)
