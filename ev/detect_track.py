...
name=['nike','bus','car','motor','person','rider','traffic light','traffic sign','train','truck']
def get_color(idx):
    idx = idx * 3
...
class Detect_Track:
    def __init__(self, tracker='ByteTrack', model_path='./models/best.pt', imgsz=(640,640), vis=True):
        ...
        if tracker == 'ByteTrack':
            ...
            self.tracker = DeepSort('./models/ckpt_car.t7', max_dist=0.1, min_confidence=0.3, nms_max_overlap=1.0, max_iou_distance=0.7, max_age=100, n_init=3, nn_budget=100, use_cuda=True)
            self._type = 'DeepSort'

        elif tracker == 'Sort':
            self.tracker = Sort(det_thresh = 0.5)
            self._type = "Sort"
                ...
                if self._type == 'ByteTrack':
                    ...
                    if len(online_targets) > 0:
                        for t in online_targets:
                            ...
                            if self.vis:
                                ...
                                cv2.putText(img_vis, name[int(t.cls.item())]+'  ID:'+str(t.track_id), (int(t.tlwh[0]),int(t.tlwh[1])), cv2.FONT_HERSHEY_COMPLEX, 0.8, color)
                    ...
                    if len(online_targets) > 0:
                        for t in online_targets:
                            ...
                            if self.vis:
                                ...
                                cv2.putText(img_vis,  name[int(cls)]+'  ID:'+str(int(tid)), (int(tlwh[0]),int(tlwh[1])), cv2.FONT_HERSHEY_COMPLEX, 0.8, color)
                    ...
                    for t in online_targets:
                        ...
                        if self.vis:
                            ...
                            cv2.rectangle(img_vis, (int(tlwh[0]), int(tlwh[1])), (int(tlwh[0]+tlwh[2]), int(tlwh[1]+tlwh[3])), color, 2)
                            cv2.putText(img_vis,  name[int(tcls)]+'  ID:'+str(int(tid)), (int(tlwh[0]),int(tlwh[1])), cv2.FONT_HERSHEY_COMPLEX, 0.8, color)
...
if __name__ =='__main__':
    ...
    model = Detect_Track(tracker='ByteTrack')
    frame_id = 0
